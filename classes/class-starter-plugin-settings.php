...
/**
 * gMap_PreventScroll_Settings Class
 *
 * @class gMap_PreventScroll_Settings
 * @version	1.0.0
 * @since 1.0.0
...
final class gMap_PreventScroll_Settings {
	...
	/**
	 * Main gMap_PreventScroll_Settings Instance
	 *
	 * Ensures only one instance of gMap_PreventScroll_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main gMap_PreventScroll_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true === (bool)apply_filters( 'google-maps-prevent-scroll-plugin-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'google-maps-prevent-scroll-plugin-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'google-maps-prevent-scroll-plugin-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= gMap_PreventScroll()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'google-maps-prevent-scroll-plugin-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'google-maps-prevent-scroll-plugin-new-line-description-fields', array( 'textarea', 'select' ) ) ) ) {
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'google-maps-prevent-scroll-plugin' );
		$settings_sections['special-fields'] = __( 'Special Fields', 'google-maps-prevent-scroll-plugin' );
		...
		return (array)apply_filters( 'google-maps-prevent-scroll-plugin-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
				...
				$settings_fields['text'] = array(
												'name' => __( 'Example Text Input', 'google-maps-prevent-scroll-plugin' ),
												'type' => 'text',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'google-maps-prevent-scroll-plugin' )
											);
				$settings_fields['textarea'] = array(
												'name' => __( 'Example Textarea', 'google-maps-prevent-scroll-plugin' ),
												'type' => 'textarea',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'google-maps-prevent-scroll-plugin' )
				...
				$settings_fields['checkbox'] = array(
												'name' => __( 'Example Checkbox', 'google-maps-prevent-scroll-plugin' ),
												'type' => 'checkbox',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'google-maps-prevent-scroll-plugin' )
				...
				$settings_fields['radio'] = array(
												'name' => __( 'Example Radio Buttons', 'google-maps-prevent-scroll-plugin' ),
												'type' => 'radio',
												'default' => '',
												'section' => 'standard-fields',
												'options' => array(
																	'one' => __( 'One', 'google-maps-prevent-scroll-plugin' ),
																	'two' => __( 'Two', 'google-maps-prevent-scroll-plugin' ),
																	'three' => __( 'Three', 'google-maps-prevent-scroll-plugin' )
															),
												'description' => __( 'Place the field description text here.', 'google-maps-prevent-scroll-plugin' )
											);
				$settings_fields['select'] = array(
													'name' => __( 'Example Select', 'google-maps-prevent-scroll-plugin' ),
													'type' => 'select',
													'default' => '',
													'section' => 'standard-fields',
													'options' => array(
			...
			case 'special-fields':
				...
				$settings_fields['select_taxonomy'] = array(
													'name' => __( 'Example Taxonomy Selector', 'google-maps-prevent-scroll-plugin' ),
		...
		return (array)apply_filters( 'google-maps-prevent-scroll-plugin-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'google-maps-prevent-scroll-plugin-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'google-maps-prevent-scroll-plugin-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'google-maps-prevent-scroll-plugin-' . $v, array() );
